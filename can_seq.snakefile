...
def mod_gb(all_changes, gb_in_file, gb_out_file, csv_out_file):
    ...
    with open(csv_out_file, 'w', newline='') as csvfile:
        ...
        spamwriter.writerow(["CDS", "Var nucl pos", "Ref nucl", "Var nucl", "Var percent", "FWD Ref coverage", "RVS Ref coverage", "FWD Var coverage", "RVS Var coverage", "Var prot pos", "Ref aa", "Var aa"])
...
rule mpileup:
    ...
    output:
    	out_mpileup = "snp/{smp}.{fa}.pileup"
    shell:
    	"samtools mpileup -f {input.fasta} -o {output.out_mpileup} {input.alignment}"

rule varscan:
    input:
        in_mpileup = "snp/{smp}.{fa}.pileup"
    output:
        "csv/{smp}.{fa}.csv"
    shell:
        "varscan mpileup2snp {input.in_mpileup} --min-var-freq {config[var_freq]} --min-coverage {config[min_cov]} --min-reads2 {config[var_reads]} > {output}"

rule gb_annot:
    input:
        csv_file = "csv/{smp}.{fa}.csv",
        ingb = "gb/{fa}.gb"
    output:
